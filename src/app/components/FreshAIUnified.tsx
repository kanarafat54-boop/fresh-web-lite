  ...
  async function unlockVault() {
    ...
    try {
      if (vaultPassphrase.trim().length >= 8) {
        ...
        setVaultPassphrase("");
        const st = freshSecureVault.status();
        setVaultMessage(
          st.hasStoredKdf
            ? "Vault unlocked with saved key params — prior sealed history can decrypt."
            : "Vault unlocked — history can be sealed at rest (key params saved on this device).",
        );
      } else {
        await conversationService.unlockEphemeral();
        setVaultMessage("Ephemeral vault unlocked for this session only (not for long-term sealed history).");
      }
  ...
  const body = <div className="fresh-ai-unified-shell">
    ...
    <div className="fresh-ai-unified-composer"><div className="fresh-ai-composer-hint"><span className="fresh-ai-hint-diamond">◇</span><small>{mode} mode · Context aware</small></div>
      <div className="fresh-ai-vault-bar" aria-label="Fresh AI vault">
        ...
        {vaultMessage ? <small className="fresh-ai-vault-msg">{vaultMessage}</small> : <small className="fresh-ai-vault-msg">At-rest E2EE when unlocked — live AI requests still use plaintext on the server.</small>}
      </div>
      ...
      <small className="fresh-ai-unified-status"><span>✦</span> {source || "Fresh AI"} · App ~99% · Vault {vaultUnlocked ? "unlocked · at-rest E2EE" : "locked"} · Weights training-required · Beyond-SI doctrine</small>
    </div>
  </div>;
