...
export default function GlobalFreshAI() {
  ...
  async function ask(event?: FormEvent) {
    ...
    setLoading(true); setError(null); setEvidence([]); setVerification(null); setSource(null); setOpen(true);
    try {
      ...
      setAnswer(payload.answer ?? "Fresh AI completed the request without a text answer."); setEvidence(Array.isArray(payload.evidence) ? payload.evidence : []); setVerification(payload.verification ?? null); setSource(payload.source ?? null);
  ...
  const content = <div className={fullscreen ? "global-fresh-ai-shell is-fullscreen" : "global-fresh-ai-shell"}>
    <div className="global-fresh-ai-header"><div><strong>Fresh AI</strong><small>Research, reasoning and proof inside Fresh</small></div><div className="global-fresh-ai-header-actions"><button type="button" onClick={() => setFullscreen((value) => !value)} aria-label={fullscreen ? "Exit full screen" : "Open full screen"}>{fullscreen ? "↙" : "↗"}</button><button type="button" onClick={closeAI} aria-label="Close Fresh AI">×</button></div></div>
