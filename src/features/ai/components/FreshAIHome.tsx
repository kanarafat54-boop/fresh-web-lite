...
function freshPrompt(prompt: string, route: string, conversation: Turn[], signal?: AbortSignal) {
  return fetch("/api/ai/ask", { method: "POST", signal, headers: { "content-type": "application/json", accept: "application/json" }, body: JSON.stringify({ goal: prompt, route, conversation: conversation.slice(-12).map(({ role, content }) => ({ role, content })) }) });
}

async function readApiResponse(response: Response): Promise<ApiResponse> {
  const raw = await response.text();
  if (!raw.trim()) throw new Error(`Fresh AI returned an empty response (${response.status}).`);
  const contentType = response.headers.get("content-type") || "";
  if (contentType.includes("application/json")) {
    try { return JSON.parse(raw) as ApiResponse; } catch { throw new Error(`Fresh AI returned invalid JSON (${response.status}).`); }
  }
  const compact = raw.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  throw new Error(compact.slice(0, 260) || `Fresh AI returned an unexpected response (${response.status}).`);
}
...
export default function FreshAIHome() {
  ...
  useEffect(() => {
    const open = (event: Event) => { const prompt = (event as CustomEvent<{ prompt?: string }>).detail?.prompt; if (prompt) setDraft(prompt); composerRef.current?.focus(); };
  ...
  useEffect(() => {
    ...
    if (!el || !userPinnedToBottomRef.current) return;
    el.scrollTo({ top: el.scrollHeight, behavior: loading ? "auto" : "smooth" });
  }, [turns, evidence, loading]);

  function onMessageScroll() {
    const el = messageListRef.current; if (!el) return;
  ...
  async function send(promptOverride?: string) {
    ...
    try {
      ...
      setEvidence(Array.isArray(payload.evidence) ? payload.evidence : []); setMeta(payload);
    } catch (reason) {
      if (reason instanceof DOMException && reason.name === "AbortError") setTurns((current) => current.filter((turn) => turn.id !== pending.id));
      else { const message = reason instanceof Error ? reason.message : "Fresh AI could not complete that request."; setError(message); setTurns((current) => current.map((turn) => turn.id === pending.id ? { ...turn, content: "", pending: false } : turn)); }
