...
export function routeIntelligence(request: IntelligenceRequest): RoutingDecision {
  ...
  const available = intelligenceConnectors
    .all()
    .filter((connector) => connector.isAvailable() && connector.tasks.includes(task));

  const selected = available[0];
  if (!selected) {
    throw new Error(`No authorized intelligence connector is available for task: ${task}`);
  }

  return {
    connectorId: selected.id,
    task,
    reason: `Selected ${selected.name} because it is available for ${task}.`,
  };
}
...
export async function runIntelligence(request: IntelligenceRequest): Promise<IntelligenceResponse> {
  ...
  return connector.run({ ...request, task: decision.task });
}
