 ...
 *
 * A connector is an explicit, authorized bridge to an external intelligence
 * provider. Cataloguing a provider never implies that Fresh AI can execute it.
 */
...
export interface IntelligenceConnector {
  ...
  readonly tasks: readonly IntelligenceTask[];
  isAvailable(): boolean;
...
export class ConnectorRegistry {
  ...
  register(connector: IntelligenceConnector): void {
    this.connectors.set(connector.id, connector);
  }

  get(id: string): IntelligenceConnector | undefined {
    return this.connectors.get(id);
  }

  all(): readonly IntelligenceConnector[] {
    return Array.from(this.connectors.values());
  }

  health(): readonly ConnectorHealth[] {
...
export function createUnavailableConnector(
  ...
  return {
    ...
    async run(): Promise<IntelligenceResponse> {
      throw new Error(`${name} connector is not configured or authorized.`);
    },
  };
}
