    ...
    <div className="fresh-flow-vertical">
      ...
      {filterOpen && (
        <div className="fresh-flow-filter-panel" aria-label="Fresh Flow filters and models">
          ...
          {FILTERS.map((item) => (
            <button key={item.id} className={filterMode === item.id ? "fresh-flow-filter-chip active" : "fresh-flow-filter-chip"} onClick={() => { setFilterMode(item.id); setFilterOpen(false); }} aria-pressed={filterMode === item.id}>
              {item.label}
            </button>
          ))}
          <span className="fresh-flow-filter-title">Models: adaptive ranking • interest signals • social graph</span>
        </div>
      )}
      {subTab === "fresh-picks" ? (
              ...
              <div key={short.id} className="fresh-flow-item" data-index={index}>
                <video
                  ref={(el) => {
                    ...
                    else { videoRefs.current.delete(index); retryCountsRef.current.delete(index); }
                  }}
                ...
                <div className="fresh-flow-short-badge" aria-hidden="true"><span>⚡</span> FRESH SHORT <i className="fresh-flow-live-dot" /></div>
                ...
                {advancedTargetId === short.id && (
                  <div className="fresh-flow-advanced-panel" role="dialog" aria-label="Short creation and collaboration actions" onClick={(e) => e.stopPropagation()}>
                    ...
                    {ADVANCED_ACTIONS.map((item) => (
                      <button key={item.id} className="fresh-flow-advanced-action" disabled={actionSending || isGuest} onClick={() => void advancedAction(short, item.id)}>
                        <span className="fresh-flow-advanced-icon">{item.icon}</span>
                ...
                {actionError && !advancedTargetId && giftTargetId !== short.id && (
                  <p className="fresh-flow-inline-error" role="alert" style={{ position: "absolute", left: 12, bottom: 12, zIndex: 5 }}>{actionError}</p>
                )}
                  ...
                  <div className="fresh-flow-gift-panel" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="fresh-flow-gift-presets">
                      {GIFT_PRESETS.map((preset) => (
                        <button key={preset.amountMinor} disabled={giftSending} onClick={() => void gift(short, preset.amountMinor)}>{preset.label} FRESH</button>
                      ))}
                    </div>
      ...
      {!immersive && (
        <section className="fresh-flow-picks" aria-label="Fresh Picks for You">
          <header className="fresh-flow-picks-header">
            <h2><span aria-hidden="true">✨</span> Fresh Picks for You</h2>
          ...
          <div className="fresh-flow-picks-rail">
            <article className="fresh-flow-pick-card"><span className="fresh-flow-pick-live">Live Now</span><strong>Gaming Live</strong><small>2.1K watching</small></article>
            <article className="fresh-flow-pick-card tone-ai"><strong>AI & Tech News</strong><small>Latest updates</small></article>
            <article className="fresh-flow-pick-card tone-travel"><strong>Travel Diaries</strong><small>Beautiful places</small></article>
            <article className="fresh-flow-pick-card tone-pod"><strong>Mindset Podcast</strong><small>New Episode</small></article>
            <article className="fresh-flow-pick-card tone-vr"><strong>VR Adventure</strong><small>Explore Worlds</small></article>
            <article className="fresh-flow-pick-card tone-hub"><strong>Creator Hub</strong><small>Tools & Tips</small></article>
          </div>
        </section>
      )}
