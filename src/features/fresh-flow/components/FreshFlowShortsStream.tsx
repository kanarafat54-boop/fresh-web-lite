...
export default function FreshFlowShortsStream({
  ...
  }, [subTab, filterMode, onImmersiveChange]);
  ...
  const advancedAction = async (short: Short, action: AdvancedAction) => {
    ...
    if (openStudioActions.includes(action)) {
      ...
      if (user && !isGuest) {
        const interactType =
          action === "remix" || action === "duet" || action === "quote" || action === "collaborate" ? action : "remix";
        try {
          await interactWithShort(user.id, short.id, interactType, {
            payload: {
              sourceShortId: short.id,
              sourceAuthorId: short.authorId,
              sourceVideoUrl: short.videoUrl,
    ...
    setActionError(null);
    try {
      await interactWithShort(user.id, short.id, "remix", {
        payload: {
          sourceShortId: short.id,
          sourceAuthorId: short.authorId,
          sourceVideoUrl: short.videoUrl,
    ...
    <div className="fresh-flow-vertical">
      <nav className={`fresh-flow-subtabs${immersive ? " hidden-immersive" : ""}`} aria-label="Fresh Flow Shorts discovery modes">
        ...
        <button
          ...
          aria-label="Open Fresh Flow filters"
      ...
      {shorts.length > 0 && (
        <div className="fresh-flow-stream" ref={containerRef} data-fresh-stream="1">
              ...
              <article key={short.id} className="fresh-flow-item" data-index={index}>
                ...
                <div className="fresh-flow-overlay">
                  <div className="fresh-flow-author-row">
                    <button type="button" className="fresh-flow-author" onClick={() => void toggleFollow(short)}>
                      <strong>@{short.authorName}</strong>
                      {!short.isFollowingAuthor && short.authorId !== user?.id && (
                        <span className="fresh-flow-follow">Follow</span>
                      )}
                    </button>
                ...
                <div className="fresh-flow-actions">
                  <ReactionPicker
                    ...
                    variant="short"
                    onReact={(type: string) => void react(short, type)}
                  />
