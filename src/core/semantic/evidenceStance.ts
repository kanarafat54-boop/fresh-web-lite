...
import type { Claim } from "./claimIntelligence";
...
const contradictionMarkers = new Set([
  "not", "never", "no", "none", "false", "denied", "denies", "denied", "opposes", "opposed",
  "failed", "fails", "rejected", "incorrect", "wrong", "debunked", "disputed", "unlikely",
]);
...
function containsNegation(text: string): boolean {
  return normalize(text).split(" ").some((token) => contradictionMarkers.has(token));
}
...
export function assessEvidenceStance(claim: Claim, evidence: SemanticEvidence): EvidenceStanceAssessment {
  ...
  const claimNegated = containsNegation(claim.statement);
  const evidenceNegated = containsNegation(evidence.claim);
  if (claimNegated !== evidenceNegated && similarity >= 0.35) {
    return { evidenceId: evidence.id, stance: "contradicts", confidence: Math.min(0.96, 0.55 + similarity * 0.4), rationale: "The evidence and claim share substantial content but differ in negation/polarity." };
  }
...
export function classifyEvidenceForClaim(claim: Claim, evidence: SemanticEvidence[]): EvidenceStanceAssessment[] {
  return evidence.map((item) => assessEvidenceStance(claim, item));
}
