...
function similarity(a: string, b: string): number {
  const aa = new Set(normalizedText(a).split(" ").filter(Boolean));
  const bb = new Set(normalizedText(b).split(" ").filter(Boolean));
...
export function compareEvidence(a: SemanticEvidence, b: SemanticEvidence): EvidenceIndependence {
  const hostA = hostOf(a.sourceUrl);
  const hostB = hostOf(b.sourceUrl);
  ...
  const titleSimilarity = similarity(a.sourceTitle ?? "", b.sourceTitle ?? "");
  const copyScore = Math.max(claimSimilarity, titleSimilarity);
...
export function buildSourceProfile(provider: string, evidence: SemanticEvidence[], reliability = 0.5): SourceProfile {
  ...
  const hosts = providerEvidence.map((item) => hostOf(item.sourceUrl)).filter(Boolean);
  return { id: `source:${provider}`, provider, canonicalHost: hosts[0], reliability: clamp(reliability), independence: 1, observations: providerEvidence.length, lastObservedAt: providerEvidence.map((item) => item.observedAt).sort().at(-1) };
}

export function weightEvidence(evidence: SemanticEvidence, profile: SourceProfile, independence = 1): EvidenceWeight {
  const rawConfidence = clamp(evidence.confidence ?? 0.5);
  const sourceReliability = clamp(profile.reliability);
  const independentFactor = clamp(independence);
  return { evidenceId: evidence.id, rawConfidence, sourceReliability, independence: independentFactor, effectiveWeight: clamp(rawConfidence * sourceReliability * independentFactor) };
}

export function effectiveEvidenceWeights(evidence: SemanticEvidence[], profiles = new Map<string, SourceProfile>()): EvidenceWeight[] {
  return evidence.map((item, index) => {
    const profile = profiles.get(item.provider) ?? buildSourceProfile(item.provider, evidence);
    let independence = 1;
    for (let i = 0; i < index; i += 1) {
      const relationship = compareEvidence(item, evidence[i]);
      independence = Math.min(independence, relationship.score);
    }
    return weightEvidence(item, profile, independence);
  });
}

export function aggregateIndependentEvidence(evidence: SemanticEvidence[], profiles = new Map<string, SourceProfile>()): number {
  const weights = effectiveEvidenceWeights(evidence, profiles);
  return clamp(1 - weights.reduce((remaining, weight) => remaining * (1 - weight.effectiveWeight), 1));
}
