...
export type Claim = { id: string; subjectEntityId: string; predicate: string; object: string; statement: string; normalizedStatement: string; observedAt: string; validFrom?: string; validTo?: string; confidence: number };
export type ClaimAssessment = { claim: Claim; relationToExisting: ClaimRelation; similarity: number; supportingEvidence: SemanticEvidence[]; counterEvidence: SemanticEvidence[]; confidence: number; rationale: string };
...
const evidenceBalance = (supporting: SemanticEvidence[], counter: SemanticEvidence[]): number => { const total = supporting.length + counter.length; return total === 0 ? 0 : (supporting.length - counter.length) / total; };
const oppositePredicate = (predicate: string): string | undefined => ({ is: "is_not", exists: "does_not_exist", founded: "not_founded", owns: "does_not_own", supports: "opposes", increases: "decreases", active: "inactive" }[normalize(predicate)]);
export function createClaim(input: Omit<Claim, "normalizedStatement">): Claim { return { ...input, normalizedStatement: normalize(`${input.predicate} ${input.object}`) }; }
export function compareClaims(left: Claim, right: Claim): { relation: ClaimRelation; similarity: number; confidence: number; rationale: string } {
  ...
  const opposite = oppositePredicate(left.predicate), directOpposition = opposite === normalize(right.predicate) || oppositePredicate(right.predicate) === normalize(left.predicate);
...
export function assessClaim(input: Omit<Claim, "normalizedStatement">): ClaimAssessment {
  const claim = createClaim(input), evidence = evidenceForClaim(claim.statement), stance = classifyEvidenceForClaim(claim, evidence);
  ...
  let relationToExisting: ClaimRelation = "unrelated", bestSimilarity = 0, rationale = "No sufficiently related existing claim was found.";
  for (const existing of semanticStore.getClaims()) {
    ...
    const candidate: Claim = { id: existing.id, subjectEntityId: existing.subjectEntityId ?? "", predicate: existing.predicate, object: String(existing.object), statement: `${existing.predicate} ${String(existing.object)}`, normalizedStatement: existing.normalizedText, observedAt: existing.lastObservedAt, validFrom: existing.validFrom, validTo: existing.validTo, confidence: existing.confidence };
  ...
  if (supportingEvidence.length && counterEvidence.length) { relationToExisting = "contradictory"; rationale = "Claim-relative evidence contains both supporting and contradictory signals; the claim remains contested."; }
  const balance = evidenceBalance(supportingEvidence, counterEvidence), sourceCount = new Set(supportingEvidence.concat(counterEvidence).map((item) => item.sourceUrl)).size;
  const diversityBonus = Math.min(0.12, Math.max(0, sourceCount - 1) * 0.03), stanceConfidence = stance.length ? stance.reduce((sum, item) => sum + item.confidence, 0) / stance.length : 0;
...
export function registerClaim(assessment: ClaimAssessment): void {
  const existing = semanticStore.getClaim(assessment.claim.id), now = assessment.claim.observedAt;
  const status: SemanticClaim["status"] = assessment.counterEvidence.length && assessment.supportingEvidence.length ? "contested" : assessment.supportingEvidence.length ? "supported" : assessment.counterEvidence.length ? "contested" : "uncertain";
  const claim: SemanticClaim = { id: assessment.claim.id, subjectEntityId: assessment.claim.subjectEntityId, predicate: assessment.claim.predicate, object: assessment.claim.object, normalizedText: assessment.claim.normalizedStatement, status, confidence: assessment.confidence, firstObservedAt: existing?.firstObservedAt ?? now, lastObservedAt: now, validFrom: assessment.claim.validFrom, validTo: assessment.claim.validTo, evidenceIds: assessment.supportingEvidence.map((item) => item.id), counterEvidenceIds: assessment.counterEvidence.map((item) => item.id) };
  semanticStore.upsertClaim(claim);
  semanticStore.upsertEntity({ id: claim.id, type: "concept", label: assessment.claim.statement, attributes: [
    { key: "subjectEntityId", value: claim.subjectEntityId ?? null, source: "ai", confidence: assessment.confidence, observedAt: now },
...
export function assessStoredClaim(claimId: string) { const claim = semanticStore.getClaim(claimId); if (!claim) return undefined; return assessClaim({ id: claim.id, subjectEntityId: claim.subjectEntityId ?? "", predicate: claim.predicate, object: String(claim.object), statement: `${claim.predicate} ${String(claim.object)}`, observedAt: claim.lastObservedAt, validFrom: claim.validFrom, validTo: claim.validTo, confidence: claim.confidence }); }
