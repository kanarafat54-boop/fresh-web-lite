...
function hasMeaningfulChange(previous: TemporalTruthAssessment, next: TemporalTruthAssessment): boolean {
  return previous.status !== next.status || Math.abs(previous.confidence - next.confidence) >= 0.05 ||
    previous.validNow !== next.validNow || previous.historical !== next.historical ||
    previous.conflictingClaimIds.join("|") !== next.conflictingClaimIds.join("|");
}
...
/**
 * Re-assesses a claim against the latest related claims without deleting history.
 * The caller owns persistence of the returned revision record.
 */
export function reviseTemporalClaim(
  claim: SemanticClaim,
  relatedClaims: SemanticClaim[],
  previous: TemporalTruthAssessment,
  now = new Date().toISOString(),
): TemporalRevision {
  const overlapping = relatedClaims.filter((other) => other.id !== claim.id && intervalsOverlap(
  ...
  return {
    ...
    previous,
    next,
    reason: classifyReason(previous, next),
    revisedAt: now,
    relatedClaimIds: overlapping.map((item) => item.id),
  };
}

export function appendRevision(history: TemporalRevisionHistory, revision: TemporalRevision): TemporalRevisionHistory {
  if (revision.reason === "no_material_change") return history;
