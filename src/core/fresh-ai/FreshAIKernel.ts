...
export class FreshAIKernel implements FreshIntelligenceEngine {
  ...
  async reason(request:FreshReasoningRequest,evidence:Evidence[]):Promise<FreshReasoningResult>{
    ...
    const base:FreshReasoningResult={
      ...
      actions:[],
      unknowns:safeEvidence.length?[]:["No external or persistent evidence was supplied to the native truth layer."],
      explanation:`Fresh AI used the native truth layer, capability composition and ${safeDimensions.length} dimensional reasoning lens(es).`,
      dimensionalReasoning:safeDimensions
    };
    return {...base,asi:evaluateASIState(request.input,base,base.plan)};
  }

  async plan(_request:FreshReasoningRequest,result:FreshReasoningResult){ return Array.isArray(result.plan)?result.plan:[]; }
  ...
  async verify(result:FreshReasoningResult){
    ...
    const verified={...result,claims,plan,dimensionalReasoning,unknowns:contradictions.length?[...new Set([...unknowns,`${contradictions.length} contradiction(s) require resolution before a definitive answer.`])]:unknowns};
