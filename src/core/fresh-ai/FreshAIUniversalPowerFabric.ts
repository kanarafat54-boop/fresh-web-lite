...
export const FRESH_AI_UNIVERSAL_POWERS: readonly FreshAIPower[] = [
  { id:"understand", name:"Understand", domain:"understanding", capabilities:["understand","chat","analyze","translate"], source:"Fresh AI Kernel / context fabric", enabled:true },
  ...
  { id:"creation", name:"Creation", domain:"creation", capabilities:["create","write","code","design","image","video","audio","generate","edit"], source:"Fresh Unified Model creation modalities", enabled:true },
  { id:"native-generation", name:"Fresh Native Generation", domain:"native-generation", capabilities:["create","generate","design","image","video","audio","edit"], source:"Fresh Neural Generation Runtime / dimensional engines", enabled:true },
  ...
  { id:"developer", name:"Engineering", domain:"developer", capabilities:["code","analyze","plan","act","review"], source:"Engineering workspace / ARA6", enabled:true },
  { id:"automation", name:"Automation", domain:"automation", capabilities:["automate","plan","act","review"], source:"Automation / agent systems", enabled:true },
  { id:"verification", name:"Verification & Truth", domain:"verification", capabilities:["verify","research","knowledge","analyze","compare"], source:"Truth / verification / evaluation systems", enabled:true },
...
export function getFreshAIPowersForCapability(capability:FreshAIUniversalCapability|string):FreshAIPower[]{const normalized=normalizeFreshAICapability(String(capability));return normalized?FRESH_AI_UNIVERSAL_POWERS.filter(power=>power.enabled&&power.capabilities.includes(normalized)):[];}
export function assertFreshAIUniversalPowerIntegrity():void{const ids=new Set<string>();for(const power of FRESH_AI_UNIVERSAL_POWERS){if(ids.has(power.id))throw new Error(`Duplicate Fresh AI power: ${power.id}`);ids.add(power.id);if(!power.capabilities.length)throw new Error(`Fresh AI power has no capabilities: ${power.id}`);}}
