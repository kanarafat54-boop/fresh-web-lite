...
/** Single capability view for the Fresh Intelligence plane. */
export function createFreshIntelligenceFabric(): FreshCapability[] {
  ...
  const registeredAgents = agentRuntime.list();
  ...
  return FRESH_AI_NATIVE_CAPABILITIES.map((id) => {
    ...
    const matchingTools = araTools
      ...
      .map((tool) => tool.id);
    const agents = FRESH_AI_AGENTS.filter((agent) => registeredAgents.some((registered) => registered.id === agent) && (
      id.includes(agent) ||
