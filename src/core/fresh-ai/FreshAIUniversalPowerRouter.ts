...
export function resolveFreshAIUniversalPowerPlan(input: { surface?: string; route: string; featureId?: string; featureName?: string; capabilities?: readonly string[]; toolNamespaces?: readonly string[]; models?: FreshAIUniversalContext["models"]; activeModelId?: string; activeVoiceModelId?: string; objectType?: string; objectId?: string; selection?: string }): FreshAIResolvedPowerPlan {
  ...
  const powers: string[] = [...new Set(capabilities.flatMap((capability) => getFreshAIPowersForCapability(capability)).map((power) => power.id))];
