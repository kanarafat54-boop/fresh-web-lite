  ...
  try {
    ...
    return {
      ...
      sources: (payload.results ?? []).filter((item) => item.title && item.url).map((item) => ({
        title: item.title as string, url: item.url as string, snippet: item.content, publishedAt: item.published_date, provider: "Tavily",
      })),
    };
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const warnings = failures.length ? [`${failures.length} research pass(es) failed or timed out; Fresh returned the successfully completed evidence.`] : [];
    return Response.json({ answer: synthesis.answer, sources: synthesis.sources, verification: synthesis.verification, warnings, mode, searchedAt: new Date().toISOString() });
  } catch (error) {
