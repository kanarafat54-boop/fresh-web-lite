...
async function providerAnswer(goal: string, route: string, intent: string, evidence: Evidence[], dimensionalContext: string, plan: string): Promise<string | null> {
  ...
  const response = await fetch(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ systemInstruction: { parts: [{ text: "You are Fresh AI, the universal intelligence and action layer inside Fresh Web Lite. Understand the user's actual goal, including vague, conversational, implicit and multi-step requests. Decide what the user needs and fulfill it directly when the available Fresh capabilities allow it. You coordinate reasoning, research, memory, planning, verification and ARA6 execution. Never claim an external action happened unless it actually happened. Use evidence when supplied, distinguish fact from inference, preserve uncertainty and conflicts, and never fabricate verification. Never expose raw URLs, citations, domains or a source directory; Fresh presents provenance through its Proof and Evidence layer." }] }, contents: [{ role: "user", parts: [{ text: `Workspace: ${route}\nInterpreted intent: ${intent}\nUser request: ${goal}\nReasoning context: ${dimensionalContext}\nPlanned capability path: ${plan}${evidenceText}` }] }], generationConfig: { temperature: 0.2, maxOutputTokens: 1400 } }) });
  if (!response.ok) return null;
  const payload = await response.json() as GeminiResponse;
  return payload.candidates?.[0]?.content?.parts?.map((part) => part.text || "").join("").trim() || null;
}

function nativeAnswer(goal: string, route: string, intent: string, dimensions: number): string {
  const normalized = goal.toLowerCase();
  if (normalized.includes("what can you do") || normalized.includes("help")) return `Fresh AI is active in ${route}. I understand requests as goals, choose the required Fresh capabilities, reason across ${dimensions} dimensions, and can coordinate research, creation, coding, planning, learning and permitted actions.`;
  if (normalized.includes("where") || normalized.includes("find")) return `I understand this as a ${intent} request. Fresh AI can route it through Fresh Search and return a synthesized result with internal proof.`;
  return `I understand this as a ${intent} request: ${goal}. I can work through the required capability path while preserving anything that needs evidence, approval or an unavailable tool.`;
}
...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    const answer = await providerAnswer(goal, route, understood.intent, evidence.sources, dimensionalContext, planText).catch(() => null);
    ...
    return json({
      answer: answer ?? nativeAnswer(goal, route, understood.intent, dimensions.length),
