-- Finalize transfer concurrency semantics without rewriting prior migrations.
...
begin
  ...
  -- Idempotency is resolved before balance validation. A network retry must
  -- return the original transaction instead of attempting to spend again.
  ...
  -- Always lock the two accounts in UUID order to prevent opposite-direction
  -- transfers from deadlocking each other.
  perform 1
...
exception
  when unique_violation then
    -- A concurrent retry won the idempotency race. Return its transaction.
    select id into v_transaction_id
    from public.treasury_transactions
    where idempotency_key = p_idempotency_key
      and created_by = auth.uid();
